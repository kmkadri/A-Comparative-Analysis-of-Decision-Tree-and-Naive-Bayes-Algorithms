% Code for Naive Bayes Training Model 
% Reading data 
...
%% Code for decision Tree Training Model
...
classificationTree = fitctree(Train_set_x, Train_set_y )

pre_train=predict(classificationTree,Train_set_x)
A=table2array(Train_set_y)
confusionchart(pre_train,A)
matrix = confusionmat(pre_train,A)
matrix=matrix'
...
%tunning my mdl
% Got the code from https://uk.mathworks.com/help/stats/fitctree.html
tunedmdl = fitctree(Train_set_x, Train_set_y,'OptimizeHyperparameters','auto')

%%  accuracy for train set
pre_train=predict(tunedmdl,Train_set_x)
A=table2array(Train_set_y)
confusionchart(pre_train,A)
matrix = confusionmat(pre_train,A)
matrix=matrix'
...
%%
% to view the tree 
view(tunedmdl, 'mode', 'Graph')
save('Final_model_2.mat','classificationTree')
